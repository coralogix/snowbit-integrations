...
resource "aws_cloudwatch_event_rule" "this" {
  ...
  event_pattern  = lookup(local.event_pattern_map, each.key)
  event_bus_name = each.key == "auth0" && length(var.custom_event_bus_name) > 0 ? var.custom_event_bus_name : "default"
