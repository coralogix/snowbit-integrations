...
"""
        with open(f"/home/ubuntu/{key}.conf", "w") as file:
            file.write(conf)
        command = f"docker run -d --user root --name {key} -v {value['log_file_path']}:/fluentd/log/file.log -v /home/ubuntu/{key}.conf:/fluentd/etc/fluent.conf fluent/fluentd:edge-debian"
